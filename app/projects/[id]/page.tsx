...
function Modal({ onClose, children }: { onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }} style={{
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--border2)", borderRadius: "4px", width: "100%", maxWidth: "480px", overflow: "hidden", animation: "slideUp 0.18s ease", maxHeight: "90vh", overflowY: "auto" }}>
        {children}
      </div>
      <style>{`
...
const [bulkFeatureId, setBulkFeatureId] = useState<string>("unassigned");
  ...
  function openVersionModal(v: Version) {
    ...
    setModalVersionNotes(v.notes || ""); setModalVersionStatus(v.status);
  }
  ...
  async function saveVersionModal() {
    ...
    await patchProject({ ...project, versions: project.versions.map(v => v.id === versionModal.id ? { ...v, number: modalVersionNumber, title: modalVersionTitle, notes: modalVersionNotes, status: modalVersionStatus } : v) });
    setVersionModal(null);
  }
  async function saveFeatureModal() {
    if (!project || !featureModal) return;
    await patchProject({ ...project, versions: project.versions.map(v => v.id === featureModal.versionId ? { ...v, features: v.features.map(f => f.id === featureModal.feature.id ? { ...f, name: modalFeatureName, notes: modalFeatureNotes, status: modalFeatureStatus } : f) } : v) });
...
async function toggleTask(taskId: string) {
  ...
  const InfoIcon = ({ onClick, color = "var(--muted)" }: { onClick: (e: React.MouseEvent) => void; color?: string }) => (
    <button onClick={e => { e.stopPropagation(); onClick(e); }} style={{ width: "18px", height: "18px", borderRadius: "50%", border: `1px solid ${color}50`, background: "none", color, cursor: "pointer", fontSize: "9px", fontStyle: "italic", fontFamily: "serif", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, padding: 0, lineHeight: 1 }}>i</button>
  );
  ...
  const StatusPills = ({ value, onChange, colors }: { value: string; onChange: (s: any) => void; colors: Record<string, string> }) => (
    <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
      {Object.keys(colors).map(s => (
        <button key={s} onClick={() => onChange(s)} style={{ padding: "6px 12px", borderRadius: "2px", fontSize: "9px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", cursor: "pointer", fontFamily: "var(--font-jetbrains)", background: value === s ? `${colors[s]}20` : "transparent", border: `1px solid ${value === s ? colors[s] : "var(--border2)"}`, color: value === s ? colors[s] : "var(--muted)", minHeight: "36px" }}>{s}</button>
      ))}
    </div>
  );
  ...
  return (
    <>
      {/* MOBILE MENU OVERLAY */}
      {mobileMenuOpen && (
        <div onClick={() => setMobileMenuOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 200, backdropFilter: "blur(4px)" }} />
      )}
      ...
      <div style={{
        ...
        zIndex: 201, transform: mobileMenuOpen ? "translateX(0)" : "translateX(100%)",
        transition: "transform 0.25s ease", padding: "20px",
        display: "flex", flexDirection: "column", gap: "10px", overflowY: "auto",
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
          <span style={{ fontFamily: "var(--font-syne)", fontSize: "12px", fontWeight: 800, letterSpacing: "2px", color: "var(--cyan)" }}>OPTIONS</span>
          <button onClick={() => setMobileMenuOpen(false)} style={{ background: "none", border: "none", color: "var(--muted)", fontSize: "20px", cursor: "pointer" }}>✕</button>
        </div>

        {/* Status selector in menu */}
...
setMobileMenuOpen(false); }} style={{ padding: "8px 14px", borderRadius: "2px", fontSize: "10px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", cursor: "pointer", fontFamily: "var(--font-jetbrains)", background: project.status === s ? `${STATUS_COLORS[s]}20` : "transparent", border: `1px solid ${project.status === s ? STATUS_COLORS[s] : "var(--border)"}`, color: project.status === s ? STATUS_COLORS[s] : "var(--muted)" }}>{s}</button>
        ...
        {/* Priority selector in menu */}
        <div style={{ marginBottom: "4px" }}>
          <div style={{ fontSize: "9px", color: "var(--muted)", letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: "8px" }}>Priority</div>
          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
            {["CRITICAL", "HIGH", "NORMAL", "BACKLOG"].map(p => (
              <button key={p} onClick={() => patchProject({ ...project, priority: p })} style={{ padding: "8px 10px", borderRadius: "2px", fontSize: "9px", fontWeight: 700, letterSpacing: "0.5px", fontFamily: "var(--font-jetbrains)", cursor: "pointer", background: (project.priority || "NORMAL") === p ? `${priorityColors[p]}20` : "transparent", border: `1px solid ${(project.priority || "NORMAL") === p ? priorityColors[p] : "var(--border)"}`, color: (project.priority || "NORMAL") === p ? priorityColors[p] : "var(--muted)" }}>{p}</button>
            ))}
          </div>
        </div>
      ...
      {versionModal && (
        <Modal onClose={() => setVersionModal(null)}>
          ...
          <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div><div style={{ fontSize: "9px", letterSpacing: "2px", color: "var(--muted)", textTransform: "uppercase", marginBottom: "4px" }}>Version</div><div style={{ fontFamily: "var(--font-syne)", fontSize: "16px", fontWeight: 700 }}>v{versionModal.number} — {versionModal.title}</div></div>
            <button onClick={() => setVersionModal(null)} style={{ background: "none", border: "1px solid var(--border2)", color: "var(--muted)", cursor: "pointer", width: "32px", height: "32px", borderRadius: "2px", fontSize: "14px" }}>✕</button>
          </div>
          <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            <div><label style={labelStyle}>Notes / Description</label><textarea value={modalVersionNotes} onChange={e => setModalVersionNotes(e.target.value)} rows={4} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.6 }} /></div>
          ...
          <div style={{ padding: "14px 24px", borderTop: "1px solid var(--border)", display: "flex", gap: "8px", justifyContent: "flex-end" }}>
            <button onClick={() => setVersionModal(null)} style={{ padding: "8px 16px", background: "none", border: "1px solid var(--border2)", color: "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", cursor: "pointer", borderRadius: "2px" }}>CANCEL</button>
            <button onClick={saveVersionModal} style={{ padding: "8px 20px", background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.3)", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", cursor: "pointer", borderRadius: "2px" }}>SAVE CHANGES</button>
          </div>
        </Modal>
      )}
      ...
      {featureModal && (
        <Modal onClose={() => setFeatureModal(null)}>
          ...
          <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div><div style={{ fontSize: "9px", letterSpacing: "2px", color: "var(--muted)", textTransform: "uppercase", marginBottom: "4px" }}>Feature</div><div style={{ fontFamily: "var(--font-syne)", fontSize: "16px", fontWeight: 700 }}>{featureModal.feature.name}</div><div style={{ fontSize: "10px", color: "var(--muted)", marginTop: "2px" }}>under <span style={{ color: "var(--cyan)" }}>{featureModal.versionLabel}</span></div></div>
            <button onClick={() => setFeatureModal(null)} style={{ background: "none", border: "1px solid var(--border2)", color: "var(--muted)", cursor: "pointer", width: "32px", height: "32px", borderRadius: "2px", fontSize: "14px" }}>✕</button>
          </div>
          <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            <div><label style={labelStyle}>Notes / Description</label><textarea value={modalFeatureNotes} onChange={e => setModalFeatureNotes(e.target.value)} rows={4} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.6 }} /></div>
          ...
          <div style={{ padding: "14px 24px", borderTop: "1px solid var(--border)", display: "flex", gap: "8px", justifyContent: "flex-end" }}>
            <button onClick={() => setFeatureModal(null)} style={{ padding: "8px 16px", background: "none", border: "1px solid var(--border2)", color: "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", cursor: "pointer", borderRadius: "2px" }}>CANCEL</button>
            <button onClick={saveFeatureModal} style={{ padding: "8px 20px", background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.3)", color: "var(--purple)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", cursor: "pointer", borderRadius: "2px" }}>SAVE CHANGES</button>
          </div>
        </Modal>
      )}
      ...
      {taskModal && project && (
        <Modal onClose={() => setTaskModal(null)}>
          ...
          <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div style={{ flex: 1, paddingRight: "12px" }}><div style={{ fontSize: "9px", letterSpacing: "2px", color: "var(--muted)", textTransform: "uppercase", marginBottom: "4px" }}>Task</div><div style={{ fontFamily: "var(--font-syne)", fontSize: "15px", fontWeight: 700 }}>{taskModal.description}</div></div>
            <button onClick={() => setTaskModal(null)} style={{ background: "none", border: "1px solid var(--border2)", color: "var(--muted)", cursor: "pointer", width: "32px", height: "32px", borderRadius: "2px", fontSize: "14px", flexShrink: 0 }}>✕</button>
          </div>
          <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: "14px" }}>
            ...
            <div><label style={labelStyle}>Notes</label><textarea value={modalTaskNotes} onChange={e => setModalTaskNotes(e.target.value)} rows={4} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.6 }} /></div>
            <div>
            ...
            <div>
              ...
              <div onClick={() => setModalTaskDone(d => !d)} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px", background: "var(--surface3)", border: "1px solid var(--border2)", borderRadius: "2px", cursor: "pointer" }}>
                <div style={{ width: "18px", height: "18px", borderRadius: "2px", flexShrink: 0, border: modalTaskDone ? "2px solid var(--cyan)" : "2px solid var(--border2)", background: modalTaskDone ? "var(--cyan)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "10px", color: "var(--bg)" }}>{modalTaskDone ? "✓" : ""}</div>
          ...
          <div style={{ padding: "14px 24px", borderTop: "1px solid var(--border)", display: "flex", gap: "8px", justifyContent: "flex-end" }}>
            <button onClick={() => setTaskModal(null)} style={{ padding: "8px 16px", background: "none", border: "1px solid var(--border2)", color: "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", cursor: "pointer", borderRadius: "2px" }}>CANCEL</button>
            <button onClick={saveTaskModal} style={{ padding: "8px 20px", background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.3)", color: "var(--green)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", cursor: "pointer", borderRadius: "2px" }}>SAVE CHANGES</button>
          </div>
        </Modal>
      )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", minWidth: 0 }}>
            ...
            {editingName ? (
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                <input value={nameVal} onChange={e => setNameVal(e.target.value)} style={{ background: "var(--surface2)", border: "1px solid rgba(0,212,255,0.4)", color: "var(--text)", fontFamily: "var(--font-syne)", fontSize: "13px", fontWeight: 700, padding: "4px 8px", borderRadius: "2px", outline: "none", width: "140px" }} />
                <button onClick={async () => { await patchProject({ ...project, name: nameVal }); setEditingName(false); }} style={{ background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.3)", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "10px", padding: "4px 8px", borderRadius: "2px", cursor: "pointer" }}>OK</button>
                <button onClick={() => setEditingName(false)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: "14px" }}>✕</button>
              </div>
            ) : (
              <div style={{ display: "flex", alignItems: "center", gap: "6px", minWidth: 0 }}>
                <span style={{ fontFamily: "var(--font-syne)", fontSize: "13px", fontWeight: 700, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{project.name}</span>
                <button onClick={() => setEditingName(true)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: "10px", flexShrink: 0 }}>✎</button>
              </div>
            )}
          </div>

          {/* DESKTOP RIGHT NAV */}
          <div className="ops-desktop-nav" style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            {editingVersion ? (
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                <input value={versionVal} onChange={e => setVersionVal(e.target.value)} style={{ background: "var(--surface2)", border: "1px solid rgba(0,212,255,0.4)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "3px 8px", borderRadius: "2px", outline: "none", width: "70px" }} />
                <button onClick={async () => { await patchProject({ ...project, version: versionVal }); setEditingVersion(false); }} style={{ background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.3)", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "10px", padding: "3px 8px", borderRadius: "2px", cursor: "pointer" }}>OK</button>
...
setEditingStatus(false); }} style={{ display: "block", width: "100%", padding: "10px 16px", textAlign: "left", background: project.status === s ? "var(--cyan-dim)" : "transparent", border: "none", color: project.status === s ? "var(--cyan)" : "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", letterSpacing: "1px", textTransform: "uppercase", cursor: "pointer" }}>{s}</button>
          ...
          {/* MOBILE RIGHT NAV */}
          <div className="ops-mobile-nav" style={{ display: "none", alignItems: "center", gap: "8px" }}>
            <span style={{ fontSize: "10px", padding: "4px 10px", borderRadius: "2px", fontWeight: 700, background: `${statusColor}18`, border: `1px solid ${statusColor}40`, color: statusColor }}>{project.status}</span>
            <button onClick={() => setMobileMenuOpen(true)} style={{ background: "none", border: "1px solid var(--border)", color: "var(--cyan)", padding: "7px 12px", borderRadius: "2px", cursor: "pointer", fontSize: "16px", lineHeight: 1 }}>☰</button>
          </div>
        </header>
        ...
        <div style={{ padding: "20px 16px", maxWidth: "1200px", margin: "0 auto" }}>
          ...
          {/* KPI ROW */}
          <div className="ops-kpi-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", marginBottom: "20px" }}>
            {[
              { label: "Progress", value: `${pct}%`, color: "var(--cyan)", sub: `${completedPhases}/${allPhases.length} phases` },
              { label: "Features", value: `${completedFeatures}/${allFeatures.length}`, color: "var(--green)", sub: "complete" },
              { label: "Versions", value: project.versions?.length || 0, color: "var(--purple)", sub: "milestones" },
              { label: "Stack", value: project.tech_stack?.length || 0, color: "var(--amber)", sub: "technologies" },
            ].map(kpi => (
              <div key={kpi.label} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", padding: "14px 16px", position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ fontSize: "9px", color: "var(--muted)", letterSpacing: "1px", textTransform: "uppercase", marginBottom: "6px" }}>{kpi.label}</div>
                <div style={{ fontFamily: "var(--font-syne)", fontSize: "22px", fontWeight: 800, color: kpi.color, lineHeight: 1, marginBottom: "2px" }}>{kpi.value}</div>
                <div style={{ fontSize: "9px", color: "var(--muted)" }}>{kpi.sub}</div>
              </div>
            ))}
          </div>

          {/* PROGRESS BAR */}
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", padding: "16px 20px", marginBottom: "20px" }}>
            ...
            <div style={{ background: "var(--surface3)", height: "6px", borderRadius: "3px", overflow: "visible" }}>
              <div style={{ height: "100%", background: "linear-gradient(90deg, var(--cyan), var(--purple))", width: `${pct}%`, transition: "width 0.5s ease", borderRadius: "3px", boxShadow: "0 0 8px 2px rgba(0,212,255,0.6), 0 0 18px 4px rgba(123,79,255,0.35)", animation: "barBreathe 2.6s ease-in-out infinite alternate" }} />
            </div>
          </div>

          {/* MAIN CONTENT — stacks on mobile */}
          <div className="ops-main-grid" style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "20px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                ...
                <div style={{ padding: "16px 20px" }}>
                  {editingSummary ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      <textarea value={summary} onChange={e => setSummary(e.target.value)} rows={4} style={{ width: "100%", background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: "2px", padding: "10px 12px", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", outline: "none", resize: "vertical", lineHeight: 1.6, boxSizing: "border-box" }} />
                      <button onClick={async () => { await patchProject({ ...project, description: summary }); setEditingSummary(false); }} style={{ alignSelf: "flex-start", padding: "8px 16px", background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.3)", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", cursor: "pointer", borderRadius: "2px" }}>SAVE</button>
                    </div>
                  ) : (
              ...
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
                <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontFamily: "var(--font-syne)", fontSize: "12px", fontWeight: 700, letterSpacing: "1px", color: "var(--purple)" }}>{project.project_type && project.project_type !== "software" ? "RESOURCES" : "TECH STACK"}</span>
                ...
                <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: "16px" }}>
                  ...
                  {editingTechStack && (
                    <div style={{ borderTop: "1px solid var(--border)", paddingTop: "16px", display: "flex", flexDirection: "column", gap: "8px" }}>
                      ...
                      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                        <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: selectedCategory ? "var(--text)" : "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px", borderRadius: "2px", outline: "none", flex: 1 }}>
                          <option value="">Select category</option>
                          {(project.tech_stack_grouped || []).map(c => <option key={c.category} value={c.category}>{c.category}</option>)}
                        </select>
                        <input value={newTechItem} onChange={e => setNewTechItem(e.target.value)} placeholder="Technology name" onKeyDown={e => e.key === "Enter" && addTechItem()} style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 10px", borderRadius: "2px", outline: "none", flex: 2 }} />
                        <button onClick={addTechItem} style={{ background: "rgba(139,92,246,0.1)", border: "1px solid rgba(139,92,246,0.3)", color: "var(--purple)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 12px", borderRadius: "2px", cursor: "pointer" }}>Add</button>
                      </div>
                      <div style={{ display: "flex", gap: "6px" }}>
                        <input value={newTechCategory} onChange={e => setNewTechCategory(e.target.value)} placeholder="Or add new category..." onKeyDown={e => e.key === "Enter" && addTechCategory()} style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 10px", borderRadius: "2px", outline: "none", flex: 1 }} />
                        <button onClick={addTechCategory} style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 12px", borderRadius: "2px", cursor: "pointer" }}>+ Category</button>
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
                <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <button onClick={() => setAddingVersion(true)} style={{ background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.2)", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "10px", letterSpacing: "1px", padding: "5px 10px", borderRadius: "2px", cursor: "pointer" }}>+ VERSION</button>
                </div>

                {addingVersion && (
                  <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", background: "var(--surface2)", display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    <input value={newVersionNumber} onChange={e => setNewVersionNumber(e.target.value)} placeholder="e.g. 2.0" style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", padding: "8px 10px", borderRadius: "2px", outline: "none", width: "80px" }} />
                    <input value={newVersionTitle} onChange={e => setNewVersionTitle(e.target.value)} placeholder="Title" style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", padding: "8px 10px", borderRadius: "2px", outline: "none", flex: 1 }} />
                    <button onClick={addVersion} style={{ background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.3)", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 12px", borderRadius: "2px", cursor: "pointer" }}>Add</button>
                    <button onClick={() => setAddingVersion(false)} style={{ background: "none", border: "1px solid var(--border)", color: "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 12px", borderRadius: "2px", cursor: "pointer" }}>Cancel</button>
                  </div>
                )}
                ...
                {sortVersions(project.versions || []).map(v => {
                  ...
                  return (
                    <div key={v.id} style={{ borderBottom: "1px solid var(--border)" }}>
                      <div style={{ padding: "14px 20px", display: "flex", alignItems: "center", gap: "10px", background: expanded ? "var(--surface2)" : "transparent" }}>
                        <span onClick={() => toggleVersion(v.id)} style={{ color: "var(--muted)", fontSize: "12px", cursor: "pointer", flexShrink: 0 }}>{expanded ? "▼" : "▶"}</span>
                        <span onClick={() => toggleVersion(v.id)} style={{ fontFamily: "var(--font-syne)", fontSize: "13px", fontWeight: 700, color: "var(--text)", flex: 1, cursor: "pointer" }}>v{v.number} — {v.title}</span>
                        {v.notes && <span style={{ fontSize: "9px", color: "var(--muted)" }}>📝</span>}
                        <span style={{ fontSize: "9px", padding: "2px 7px", borderRadius: "2px", fontWeight: 700, letterSpacing: "1px", background: `${vColor}18`, border: `1px solid ${vColor}40`, color: vColor, whiteSpace: "nowrap" }}>{computedStatus}</span>
                        <span style={{ fontFamily: "var(--font-syne)", fontSize: "13px", fontWeight: 800, color: vColor, minWidth: "36px", textAlign: "right", ...(computedStatus === "complete" ? { animation: "greenPulse 2.4s ease-in-out infinite alternate" } : {}) }}>{vPct}%</span>
                      ...
                      {expanded && (
                        <div style={{ padding: "0 20px 16px" }}>
                          <div style={{ marginTop: "12px" }}>
                            ...
                            {addingFeature === v.id && (
                              <div style={{ display: "flex", gap: "6px", marginBottom: "8px", flexWrap: "wrap" }}>
                                <input value={newFeatureName} onChange={e => setNewFeatureName(e.target.value)} placeholder="Feature name" onKeyDown={e => e.key === "Enter" && addFeature(v.id)} style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", padding: "8px 10px", borderRadius: "2px", outline: "none", flex: 1 }} />
                                <select value={newFeatureStatus} onChange={e => setNewFeatureStatus(e.target.value as Feature["status"])} style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px", borderRadius: "2px", outline: "none" }}>
                                  <option value="planned">Planned</option>
                                  <option value="in-progress">In Progress</option>
                                  <option value="complete">Complete</option>
                                </select>
                                <button onClick={() => addFeature(v.id)} style={{ background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.3)", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 12px", borderRadius: "2px", cursor: "pointer" }}>Add</button>
                                <button onClick={() => setAddingFeature(null)} style={{ background: "none", border: "1px solid var(--border)", color: "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 12px", borderRadius: "2px", cursor: "pointer" }}>✕</button>
                              </div>
                            )}
                            {v.features?.map(f => (
                              <div key={f.id} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                                <button onClick={() => toggleFeatureStatus(v.id, f.id)} style={{ fontSize: "9px", padding: "3px 8px", borderRadius: "2px", fontWeight: 700, letterSpacing: "0.5px", textTransform: "uppercase", cursor: "pointer", background: `${FEATURE_STATUS_COLORS[f.status]}18`, border: `1px solid ${FEATURE_STATUS_COLORS[f.status]}40`, color: FEATURE_STATUS_COLORS[f.status], fontFamily: "var(--font-jetbrains)", minWidth: "76px", textAlign: "center", minHeight: "28px" }}>{f.status}</button>
                                ...
                                {f.notes && <span style={{ fontSize: "9px", color: "var(--muted)" }}>📝</span>}
                                <InfoIcon onClick={() => openFeatureModal(f, v.id, `v${v.number} — ${v.title}`)} color="var(--purple)" />
                                <button onClick={() => deleteFeature(v.id, f.id)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: "14px", padding: "0 4px", minWidth: "24px", minHeight: "24px" }}>✕</button>
                              </div>
                            ))}
                          </div>
                          ...
                          <div style={{ marginTop: "16px" }}>
                            ...
                            {addingPhase === v.id && (
                              <div style={{ display: "flex", gap: "6px", marginBottom: "8px", flexWrap: "wrap" }}>
                                <input value={newPhaseName} onChange={e => setNewPhaseName(e.target.value)} placeholder="Phase name" onKeyDown={e => e.key === "Enter" && addPhase(v.id)} style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", padding: "8px 10px", borderRadius: "2px", outline: "none", flex: 1 }} />
                                <button onClick={() => addPhase(v.id)} style={{ background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.3)", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 12px", borderRadius: "2px", cursor: "pointer" }}>Add</button>
                                <button onClick={() => setAddingPhase(null)} style={{ background: "none", border: "1px solid var(--border)", color: "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px 12px", borderRadius: "2px", cursor: "pointer" }}>✕</button>
                              </div>
                            )}
                            {v.phases?.map((phase, i) => (
                              <div key={phase.id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 0", borderBottom: "1px solid var(--border)" }}>
                                <span style={{ fontSize: "10px", color: "var(--muted)", minWidth: "20px" }}>{String(i + 1).padStart(2, "0")}</span>
                                <div onClick={() => togglePhase(v.id, phase.id)} style={{ width: "20px", height: "20px", minWidth: "20px", borderRadius: "2px", border: phase.completed ? "2px solid var(--green)" : "2px solid var(--border2)", background: phase.completed ? "var(--green)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "10px", color: "#fff", cursor: "pointer" }}>{phase.completed ? "✓" : ""}</div>
                                ...
                                <button onClick={() => deletePhase(v.id, phase.id)} style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: "14px", padding: "0 4px", minWidth: "24px" }}>✕</button>
                              </div>
                            ))}
                          </div>
                        </div>
              ...
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
                ...
                <div style={{ padding: "16px 20px" }}>
                  {editingProgress ? (
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      <textarea value={progressVal} onChange={e => setProgressVal(e.target.value)} rows={5} style={{ width: "100%", background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: "2px", padding: "10px 12px", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", outline: "none", resize: "vertical", lineHeight: 1.6, boxSizing: "border-box" }} />
                      <button onClick={async () => { await patchProject({ ...project, current_progress: progressVal }); setEditingProgress(false); }} style={{ alignSelf: "flex-start", padding: "8px 16px", background: "rgba(16,185,129,0.1)", border: "1px solid rgba(16,185,129,0.3)", color: "var(--green)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", cursor: "pointer", borderRadius: "2px" }}>SAVE</button>
                    </div>
                  ) : (
                    <p style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.8 }}>{project.current_progress || "No progress notes yet."}</p>
                  )}
                </div>
              </div>
            </div>
      ...
      <button onClick={() => { setBulkMode(true); setAddingTask(false); setBulkText(""); }} style={{ background: "none", border: "1px solid rgba(139,92,246,0.3)", color: "var(--purple)", fontFamily: "var(--font-jetbrains)", fontSize: "10px", cursor: "pointer", letterSpacing: "1px", padding: "4px 10px", borderRadius: "2px" }}>++ BULK</button>
                ...
                {!tasksCollapsed && addingTask && (
                  <div style={{ padding: "12px 20px", borderBottom: "1px solid var(--border)", background: "var(--surface2)", display: "flex", flexDirection: "column", gap: "8px" }}>
                    <input value={newTaskDesc} onChange={e => setNewTaskDesc(e.target.value)} onKeyDown={e => e.key === "Enter" && addTask()} placeholder="Task description..." autoFocus style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", padding: "8px 10px", borderRadius: "2px", outline: "none" }} />
                    <select value={newTaskFeatureId} onChange={e => setNewTaskFeatureId(e.target.value)} style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: newTaskFeatureId !== "unassigned" ? "var(--text)" : "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", padding: "8px", borderRadius: "2px", outline: "none" }}>
                      <option value="unassigned">— No feature —</option>
                      {sortVersions(project.versions || []).map(v => (
                        <optgroup key={v.id} label={`v${v.number} — ${v.title}`}>
                          {(v.features || []).map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
                        </optgroup>
...
<div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
  ...
  <ActivityFeed projectId={id as string} limit={30} compact />
              ...
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
                ...
                <div style={{ padding: "16px 20px" }}>
                  <textarea value={blockers} onChange={e => setBlockers(e.target.value)} rows={4} style={{ width: "100%", background: "var(--surface2)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: "2px", padding: "10px 12px", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", outline: "none", resize: "vertical", lineHeight: 1.6, boxSizing: "border-box" }} placeholder="Any blockers or issues..." />
                </div>
              </div>

              <button onClick={saveNotes} disabled={saving} style={{ padding: "12px", background: saved ? "rgba(16,185,129,0.1)" : "var(--cyan-dim)", border: `1px solid ${saved ? "rgba(16,185,129,0.3)" : "rgba(0,212,255,0.3)"}`, color: saved ? "var(--green)" : "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", letterSpacing: "1px", cursor: "pointer", borderRadius: "2px", width: "100%", minHeight: "44px" }}>
              ...
              }} style={{ padding: "12px", background: "transparent", border: "1px solid rgba(239,68,68,0.2)", color: "var(--muted)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", letterSpacing: "1px", cursor: "pointer", borderRadius: "2px", width: "100%", minHeight: "44px" }}
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .ops-mobile-nav { display: flex !important; }
