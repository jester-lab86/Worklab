...
import { Project, Version, Feature, Phase, TechCategory } from "@/types";
...
const [editingStatus, setEditingStatus] = useState(false);
  ...
  async function togglePhase(vid: string, pid: string) {
    ...
    const updated = {
      ...
      versions: project.versions.map(v =>
        v.id === vid
          ? { ...v, phases: v.phases.map(p => p.id === pid ? { ...p, completed: !p.completed } : p) }
          : v
      ),
    };
  ...
  async function addFeature(vid: string) {
    ...
    const updated = {
      ...
      versions: project.versions.map(v =>
        v.id === vid
          ? { ...v, features: [...v.features, { id: uid(), name: newFeatureName.trim(), status: newFeatureStatus }] }
          : v
      ),
    };
    await patchProject(updated);
  ...
  async function deleteFeature(vid: string, fid: string) {
    ...
    const updated = {
      ...project,
      versions: project.versions.map(v =>
        v.id === vid ? { ...v, features: v.features.filter(f => f.id !== fid) } : v
      ),
    };
    await patchProject(updated);
  }
  ...
  async function addPhase(vid: string) {
    ...
    const updated = {
      ...
      versions: project.versions.map(v =>
        v.id === vid
          ? { ...v, phases: [...v.phases, { id: uid(), title: newPhaseName.trim(), completed: false }] }
          : v
      ),
    };
    await patchProject(updated);
  ...
  async function deletePhase(vid: string, pid: string) {
    ...
    const updated = {
      ...project,
      versions: project.versions.map(v =>
        v.id === vid ? { ...v, phases: v.phases.filter(p => p.id !== pid) } : v
      ),
    };
    await patchProject(updated);
  }
  ...
  async function addVersion() {
    ...
    const newV: Version = {
      id: uid(),
      number: newVersionNumber.trim(),
      title: newVersionTitle.trim() || `Version ${newVersionNumber.trim()}`,
      status: "planned",
      features: [],
      phases: [],
    };
    ...
    <div style={{ position: "relative", zIndex: 1, minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          ...
          <div style={{ fontFamily: "var(--font-syne)", fontSize: "14px", fontWeight: 700, color: "var(--text)" }}>
            {project.name}
          </div>
        </div>
  ...
  {editingStatus && (
    <div style={{
      position: "absolute", top: "32px", right: 0, background: "var(--surface2)",
      border: "1px solid var(--border2)", borderRadius: "4px", overflow: "hidden", zIndex: 200, minWidth: "140px",
    }}>
      {(["concept", "building", "launched"] as const).map(s => (
        <button
          ...
          onClick={async () => {
            if (!project) return;
            await patchProject({ ...project, status: s });
            setEditingStatus(false);
          }}
          style={{
            display: "block", width: "100%", padding: "10px 16px", textAlign: "left",
            background: project.status === s ? "var(--cyan-dim)" : "transparent",
            border: "none", color: project.status === s ? "var(--cyan)" : "var(--text)",
            fontFamily: "var(--font-jetbrains)", fontSize: "11px", letterSpacing: "1px",
            textTransform: "uppercase", cursor: "pointer",
          }}
          onMouseEnter={e => e.currentTarget.style.background = "var(--cyan-dim)"}
          onMouseLeave={e => e.currentTarget.style.background = project.status === s ? "var(--cyan-dim)" : "transparent"}
        >
          {s}
        </button>
      ))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "16px", marginBottom: "24px" }}>
          {[
            ...
            <div key={kpi.label} style={{
              ...
              borderRadius: "4px", padding: "20px", position: "relative", overflow: "hidden",
            }}>
    ...
    {editingSummary ? (
      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
        ...
        <button
          onClick={async () => {
            if (!project) return;
            await patchProject({ ...project, description: summary });
            setEditingSummary(false);
          }}
          style={{ alignSelf: "flex-start", padding: "6px 16px", background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.3)", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)", fontSize: "11px", letterSpacing: "1px", cursor: "pointer", borderRadius: "2px" }}
        >
          SAVE
        </button>
      </div>
      ...
      <p style={{ fontSize: "13px", color: "var(--muted)", lineHeight: 1.8 }}>
        {project.description || <span style={{ opacity: 0.4 }}>No description yet. Click EDIT to add one.</span>}
      </p>
    )}
  </div>
</div>
            {/* TECH STACK GROUPED */}
            {project.tech_stack_grouped?.length > 0 && (
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
                <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", fontFamily: "var(--font-syne)", fontSize: "12px", fontWeight: 700, letterSpacing: "1px", color: "var(--purple)" }}>
                  TECH STACK
                </div>
                    ...
                    <div key={i}>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                        {cat.items.map((item, j) => (
                          <span key={j} style={{
                            padding: "4px 10px", borderRadius: "2px", fontSize: "11px",
                            background: "rgba(139,92,246,0.08)", border: "1px solid rgba(139,92,246,0.2)", color: "var(--purple)",
                          }}>
                            {item}
                          </span>
                        ))}
            ...
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
              <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button onClick={() => setAddingVersion(true)} style={{
                  background: "var(--cyan-dim)", border: "1px solid rgba(0,212,255,0.2)", color: "var(--cyan)",
              ...
              {addingVersion && (
                <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)", background: "var(--surface2)", display: "flex", gap: "8px", flexWrap: "wrap" }}>
                  ...
                  <input
                    ...
                    placeholder="Title e.g. User Accounts"
              ...
              {(project.versions || []).map(v => {
                ...
                return (
                  <div key={v.id} style={{ borderBottom: "1px solid var(--border)" }}>
                    {/* VERSION HEADER */}
                    <div onClick={() => toggleVersion(v.id)} style={{
                      padding: "14px 20px", display: "flex", alignItems: "center", gap: "12px",
                      cursor: "pointer", background: expanded ? "var(--surface2)" : "transparent",
                      transition: "background 0.2s",
                    }}>
                      <span style={{ color: "var(--muted)", fontSize: "12px" }}>{expanded ? "▼" : "▶"}</span>
                      <span style={{ fontFamily: "var(--font-syne)", fontSize: "14px", fontWeight: 700, color: "var(--text)", flex: 1 }}>
                        v{v.number} — {v.title}
                      </span>
                      <span style={{
                        fontSize: "9px", padding: "2px 8px", borderRadius: "2px", fontWeight: 700,
                        letterSpacing: "1px", textTransform: "uppercase",
                        background: `${vColor}18`, border: `1px solid ${vColor}40`, color: vColor,
                      }}>
                        {v.status}
                      </span>
                      <span style={{ fontFamily: "var(--font-syne)", fontSize: "14px", fontWeight: 800, color: vColor, minWidth: "40px", textAlign: "right" }}>
                        {vPct}%
                      </span>
                    </div>
                      ...
                      <div style={{ padding: "0 20px 16px" }}>
                        ...
                        <div style={{ marginTop: "12px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                            ...
                            <button onClick={() => setAddingFeature(v.id)} style={{
                              background: "none", border: "none", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)",
                          ...
                          {addingFeature === v.id && (
                            <div style={{ display: "flex", gap: "6px", marginBottom: "8px", flexWrap: "wrap" }}>
                              <input
                                ...
                                onKeyDown={e => e.key === "Enter" && addFeature(v.id)}
                                style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", padding: "6px 10px", borderRadius: "2px", outline: "none", flex: 1 }}
                              />
                              <select
                                value={newFeatureStatus}
                                onChange={e => setNewFeatureStatus(e.target.value as Feature["status"])}
                          ...
                          {v.features?.map(f => (
                            <div key={f.id} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "7px 0", borderBottom: "1px solid var(--border)" }}>
                              <button onClick={() => toggleFeatureStatus(v.id, f.id)} style={{
                                fontSize: "9px", padding: "2px 8px", borderRadius: "2px", fontWeight: 700,
                                letterSpacing: "0.5px", textTransform: "uppercase", cursor: "pointer",
                                background: `${FEATURE_STATUS_COLORS[f.status]}18`,
                                border: `1px solid ${FEATURE_STATUS_COLORS[f.status]}40`,
                                color: FEATURE_STATUS_COLORS[f.status],
                                fontFamily: "var(--font-jetbrains)", minWidth: "80px", textAlign: "center",
                              }}>
                                {f.status}
                              </button>
                              <span style={{ fontSize: "12px", color: f.status === "complete" ? "var(--muted)" : "var(--text)", textDecoration: f.status === "complete" ? "line-through" : "none", flex: 1 }}>
                                {f.name}
                              </span>
                        ...
                        <div style={{ marginTop: "16px" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                            ...
                            <button onClick={() => setAddingPhase(v.id)} style={{
                              background: "none", border: "none", color: "var(--cyan)", fontFamily: "var(--font-jetbrains)",
                              fontSize: "10px", cursor: "pointer", letterSpacing: "1px",
                            }}>+ ADD</button>
                          </div>
                          ...
                          {addingPhase === v.id && (
                            <div style={{ display: "flex", gap: "6px", marginBottom: "8px" }}>
                              <input
                                ...
                                onKeyDown={e => e.key === "Enter" && addPhase(v.id)}
                                style={{ background: "var(--surface3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "var(--font-jetbrains)", fontSize: "12px", padding: "6px 10px", borderRadius: "2px", outline: "none", flex: 1 }}
                              />
                          ...
                          {v.phases?.map((phase, i) => (
                            <div key={phase.id} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
                              ...
                              <div onClick={() => togglePhase(v.id, phase.id)} style={{
                                width: "18px", height: "18px", minWidth: "18px", borderRadius: "2px",
                                border: phase.completed ? "2px solid var(--green)" : "2px solid var(--border2)",
                                background: phase.completed ? "var(--green)" : "transparent",
                                display: "flex", alignItems: "center", justifyContent: "center",
                                fontSize: "10px", color: "#fff", cursor: "pointer", transition: "all 0.2s",
                              }}>
                                {phase.completed ? "✓" : ""}
                              </div>
                              <span style={{ fontSize: "12px", fontFamily: "var(--font-syne)", fontWeight: 600, flex: 1, color: phase.completed ? "var(--muted)" : "var(--text)", textDecoration: phase.completed ? "line-through" : "none" }}>
                                {phase.title}
                              </span>
            ...
            {project.current_progress && (
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
                <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", fontFamily: "var(--font-syne)", fontSize: "12px", fontWeight: 700, letterSpacing: "1px", color: "var(--green)" }}>
                  CURRENT PROGRESS
                </div>
                <div style={{ padding: "16px 20px", fontSize: "13px", color: "var(--muted)", lineHeight: 1.8 }}>
                  {project.current_progress}
                </div>
              </div>
            )}
...
<div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
      ...
      return (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 0", borderBottom: i < (project.still_to_complete.length - 1) ? "1px solid var(--border)" : "none" }}>
          <div
            ...
            style={{ width: "16px", height: "16px", minWidth: "16px", borderRadius: "2px", border: done ? "2px solid var(--cyan)" : "2px solid var(--border2)", background: done ? "var(--cyan)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "9px", color: "var(--bg)", transition: "all 0.2s", cursor: "pointer" }}
          >
            {done ? "✓" : ""}
          </div>
          ...
          <button
            ...
            style={{ background: "none", border: "none", color: "var(--muted)", cursor: "pointer", fontSize: "12px", padding: "0 4px" }}
          >✕</button>
        </div>
      );
    })}
  ...
  style={{
    ...
    border: "1px solid rgba(239,68,68,0.2)", color: "var(--muted)",
    fontFamily: "var(--font-jetbrains)", fontSize: "11px", letterSpacing: "1px",
    cursor: "pointer", borderRadius: "2px", transition: "all 0.2s", width: "100%",
  }}
  onMouseEnter={e => {
