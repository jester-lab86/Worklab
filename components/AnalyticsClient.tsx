    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "var(--font-jetbrains)" }}>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          <Link href="/dashboard" style={{ fontFamily: "var(--font-jetbrains)", fontSize: "11px", color: "var(--muted)", textDecoration: "none", letterSpacing: "1px" }}
      ...
      <div style={{ padding: "32px", maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden", marginBottom: "20px" }}>
          <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)" }}>
              ...
              return (
                <div key={p.id}>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "6px" }}>
                    <Link href={`/projects/${p.id}`} style={{ fontFamily: "var(--font-syne)", fontSize: "13px", fontWeight: 700, color: "var(--text)", textDecoration: "none", flex: 1, transition: "color 0.15s" }}
                      onMouseEnter={e => e.currentTarget.style.color = "var(--cyan)"}
                      onMouseLeave={e => e.currentTarget.style.color = "var(--text)"}
                    >{p.name}</Link>
                    <span style={{ fontSize: "9px", padding: "2px 7px", borderRadius: "2px", fontWeight: 700, letterSpacing: "1px", background: `${statusColor}18`, border: `1px solid ${statusColor}40`, color: statusColor }}>{p.status.toUpperCase()}</span>
                    <span style={{ fontSize: "10px", color: "var(--muted)" }}>{tasks.done}/{tasks.total} tasks</span>
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "4px", overflow: "hidden" }}>
          ...
          <div style={{ padding: "20px", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: "12px" }}>
            {projects.map(p => {
              const t = getTasks(p);
